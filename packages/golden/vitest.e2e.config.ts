 ...
 * Claude shim), the audit arm, the T201 ADK MCPToolset acceptance arm, and the
 * T204 Pi capability smoke. The golden arms gate themselves on the same env contract as the conformance harness
 * (`MXL_CONFORMANCE_TWO_DAEMON` + `MXL_CONFORMANCE_GOLDEN_POLICY` + fixture
 * coordinates, see `test/_golden-harness.ts`); the ADK arm is additionally opted in
 * with `MXL_ADK_MCP_E2E=1` and a real `google-adk` install:
