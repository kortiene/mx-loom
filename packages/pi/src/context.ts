/**
 * The binding context + session wiring (T205).
 *
 * A {@link BindingContext} is the secret-free bundle every Pi tool call is handed:
 * the injected {@link DaemonCall} (a live `MxSession` or a bare `MxClient`), the
 * session **workspace room** (from `MxSession.room`, **never** model input — the
 * model never names a Matrix room id), the session-stable `correlationId`, and the
 * {@link AuditSink} (default {@link NullAuditSink}). It holds **no** secret: the
 ...
 *
 * A local reimplementation of `@mx-loom/mcp`'s `createBindingContext` (the spec's
 * OQ #1, recommendation (a)): same shape, but it imports no MCP SDK, so the Pi
 * binding's dependency graph stays MCP-free.
 *
 * {@link createPiBindingContext} prefers opening an `MxSession` (one
 * `agent.register`, a liveness heartbeat, correlation threading) so every RPC is
 * correlation-stamped and guarded; a caller may instead inject an already-open
 * session or a bare `DaemonCall` (the unit-test / embed path). `close()` tears
 * down only what this context opened — an injected session/daemon is the caller's
 * to close.
 */
...
/**
 * The secret-free per-binding context threaded into every tool dispatch.
 *
...
/** Options for {@link createPiBindingContext}. */
export interface CreatePiBindingContextOptions {
  /**
 ...
 *
 * Resolution order:
 *  1. an injected open {@link CreatePiBindingContextOptions.session} — bound as-is;
 *  2. an injected bare {@link CreatePiBindingContextOptions.daemon} — bound with
 *     the supplied {@link CreatePiBindingContextOptions.room};
 *  3. otherwise, open a fresh `MxSession` via `openSession` (the production path) —
 *     and `close()` will close it.
 */
export async function createPiBindingContext(
  options: CreatePiBindingContextOptions = {},
