 ...
 *  - `registerMxTools` returned tool names match CANONICAL_M1_TOOLS.
 ...
 *  - Factory tool names match CANONICAL_M1_TOOLS.
...
describe('registerMxTools', () => {
  it('calls registerTool once per canonical descriptor', async () => {
    ...
    expect(registered).toHaveLength(CANONICAL_M1_TOOLS.length);
  });
  ...
  it('registered tool names match CANONICAL_M1_TOOLS names', async () => {
    const ctx = await makeCtx();
    const { host, registered } = makeFakeHost();
    registerMxTools(host, { context: ctx, builders: fakeBuilders });
...
describe('createMxPiExtension', () => {
  ...
  it('factory-registered tool names match CANONICAL_M1_TOOLS names', async () => {
    const ctx = await makeCtx();
    const factory = createMxPiExtension({ context: ctx, builders: fakeBuilders });
    const { host, registered } = makeFakeHost();
    factory(host);
  ...
  it('factory can be called multiple times on different hosts', async () => {
    ...
    factory(host1);
    factory(host2);
    expect(r1).toHaveLength(CANONICAL_M1_TOOLS.length);
    expect(r2).toHaveLength(CANONICAL_M1_TOOLS.length);
  });
});
