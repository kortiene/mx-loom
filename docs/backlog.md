...
- **Acceptance criteria:**
  - [~] Every tool result produces exactly one audit row — *mechanism landed (idempotent `dedup_key` + `ON CONFLICT DO NOTHING`); "every" staged behind the binding chokepoint (T109/T110), proven end-to-end by T114.*
  - [~] Rows correlate model action ↔ daemon invocation ↔ approval — *landed: one row carries all three legs, joinable by `invocation_id`/`correlation_id` (unit-proven; live-Postgres query path gated behind `MXL_AUDIT_PG=1`).*
- **Dependencies:** blocked-by T102
