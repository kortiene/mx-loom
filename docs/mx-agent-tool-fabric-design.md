...
| `mx_create_task` / `mx_update_task` / `mx_list_tasks` | `task.create/update/list/graph` | Author and read the shared, durable plan (DAG) — **live (T301)**: create with `depends_on`/`blocks`, list returns the DAG (nodes + edges), update transitions state; a node's signed `action` is **authored, not dispatched** (T303 dispatches) |
...
- **Canonical registry** with the **9 M1 verbs**: `mx_find_agents`, `mx_describe_agent`, `mx_delegate_tool`, `mx_run_command` (guarded, off by default), `mx_await_result`, `mx_share_context`, `mx_get_context`, `mx_cancel`, `mx_workspace_status`. (The **3 task DAG verbs** — `mx_create_task` / `mx_update_task` / `mx_list_tasks` — landed in **Phase 3 (T301)**, growing the canonical set to **12**; `mx_workspace_status` surfaces agents + project and leaves its reserved `tasks` dimension intentionally unpopulated — a dedicated `mx_list_tasks` is the cleaner surface.)
...
| **3 — Coordination depth** | Plans + context as tools | Task DAG tools (`mx_create/update/list_tasks`) — **landed (T301)**, the canonical set grows 9 → 12; `task.watch` resumption — **landed (T302)**: `SessionDescriptor` / `PlanSnapshot` / `resumeSession()` / `watchTasks()` in the toolbelt; signed task-action dispatch (T303) follows (`mx_cancel` shipped in Phase 1 via T108) | A multi-agent plan executes across ≥2 agents with durable task state surviving a runtime restart |
