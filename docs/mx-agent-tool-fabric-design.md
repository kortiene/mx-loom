...
| | |
|---|---|
| Status | Active — M0 complete (T001–T008 delivered); M1 complete; M2 complete — T101–T108 landed (both delegation surfaces: named tools + guarded exec; the shared-context publish/fetch seam; and the cancel + workspace-observe verbs — the **9-verb M1 model-facing surface is complete** at the handler layer); **T111 landed** (`@mx-loom/claude` bootstrapped — JSON Schema → Zod converter, the Claude-binding seam); **T113 landed** (`@mx-loom/audit` — the Postgres queryable-index mirror of `audit_ref`: projection + idempotent sink + best-effort tap); **T109 + T110 landed — both bindings of locked decision #1 are live**: the generated MCP server (`@mx-loom/mcp`) *and* the Claude Agent SDK in-process shim (`@mx-loom/claude`: `createSdkMcpServer()` + `tool()`, hidden poll loop, `canUseTool` HITL), each applying the T113 audit tap once at its single chokepoint; **T114 landed — the GOLDEN end-to-end gate** (`@mx-loom/golden`): one binding-agnostic S1–S8 scenario (discovery → ungated delegation → approval-gated approve/deny → guarded exec → `deny_args_regex` + deny-by-default denials) driven through **both** bindings with a genuinely out-of-band operator, asserting the T102 envelope + the secret boundary + the audit rows for each step — skip-clean locally, fail-not-skip in CI, with the live green (and live-Postgres) staged behind the golden two-daemon fixture (`MXL_CONFORMANCE_TWO_DAEMON=1` + `MXL_CONFORMANCE_GOLDEN_POLICY=1` / `MXL_AUDIT_PG=1`); **T201 landed** — ADK `MCPToolset` recipe (`examples/adk/`, T201 / #23): deny-by-default `safe_mx_mcp_env()` + all non-secret session flags (`--correlation-id`/`--cwd`/`--project-id`/`--git-commit`/`--max-invocations`) wired through `MxSessionOptions`; Python–TypeScript boundary drift guard; ADK e2e acceptance arm gated `MXL_ADK_MCP_E2E=1`; **T204 decided the Pi arm of M2**: no built-in MCP client, so Pi uses native tool registration and treats `@mx-loom/mcp` as reference-only; **T205 landed** (`@mx-loom/pi` — the Pi native binding from the canonical registry: fail-closed JSON Schema → TypeBox converter with `enum` → `StringEnum` (Google-provider-safe), locally reimplemented dispatch/context, T102 envelope in `content`+`details`, single `withAudit` tap, secret-free, TypeBox builders injected by the host; live two-daemon arm staged behind `MXL_PI_BINDING_E2E=1`); **T206 landed** (the M2-exit portability matrix — the binding-agnostic `scenario.ts` step table runs cross-runtime under Pi + ADK + OpenCode from one canonical descriptor set; Pi/ADK express the full S1–S8 model-free, OpenCode the always-on descriptor-identity invariant + S3 under an opt-in model; `packages/golden/test/portability-matrix.e2e.test.ts`, demand-gated `MXL_PORTABILITY_MATRIX=1`; skip-clean without fixture, fail-not-skip when demanded; `portability` CI job added); **T207 landed** (docs only — `docs/runtime-integration.md` + `examples/README.md` created: one copy-pasteable, gated-e2e-verified setup per runtime — ADK/Claude/OpenCode/Pi/custom; secret boundary and common contract stated once; `packages/mcp/test/runtime-guide.test.ts` drift guard pins the guide against the live registry, taxonomy, CLI, and example files; root README + design §3 + all five per-runtime READMEs cross-linked); **T301 landed** (M3 first deliverable — 3 task-DAG verbs `mx_create_task` / `mx_update_task` / `mx_list_tasks` in `@mx-loom/registry`, canonical set grows 9 → 12, `CANONICAL_TOOLS` superset introduced; unblocks T302/T303/T304); **T302 landed** (`task.watch` resumption — `@mx-loom/toolbelt`: `SessionDescriptor`/`TaskCursor` non-secret resume handle + `assertSessionDescriptor`/`serialize`/`parse` credential-guarded on both write and read; `PlanSnapshot` + `reconcile()` pure plan reconstructor (`done`/`inFlight`/`ready`/`blocked`); `resumeSession()` (validate → idempotent re-`agent.register` upsert → reconstruct plan → `{ session, plan, resumed }`); `watchTasks()` clamped-poll-fallback stream (`[250 ms, 30 s]`, cursor+signature dedup, stream-gap recovery); `MxSession.describe()` mints the descriptor; AC "a killed-and-restarted runtime resumes the plan from task state" satisfied on the landed `task.list` poll surface; push `task.watch` backend gated as a one-const swap (`TASK_WATCH_METHOD`); toolbelt stays registry-free; unblocks T303, T304); **T303 landed** (signed task-action dispatch — `@mx-loom/registry`: the new model-facing verb `mx_dispatch_task` (canonical set grows 12 → 13) resolves a node's authored `action` (`task.list` + id filter) and re-routes it through the landed `mx_delegate_tool` (`call.start`, `kind: 'tool'`) / `mx_run_command` (`exec.start`, `kind: 'exec'`) so the dispatch traverses the identical receiver-side authorize pipeline — the AC "a task action runs through the full authorize pipeline on dispatch" holds by construction; the shared pure `actionToDispatch` mapper is the single source of truth aligning what `mx_create_task` authors with what dispatch runs; authoring an action is never authorizing it (no in-process trust/policy/approval check; the forbidden-authority set is untouched), and a task-stable `idempotency_key` (`idk_task_<task_id>`) makes a re-dispatch on restart dedupe rather than double-execute; wired into MCP/Claude/Pi `DISPATCH`; the new wire shapes staged behind `MXL_CONFORMANCE_TWO_DAEMON=1`; unblocks T304); see `docs/backlog.md` |
| Date | 2026-06-25 |
...
The daemon has ~45 RPC methods; most are operator/crypto/lifecycle plumbing that must **never** be a model tool. The model-facing set is ~13 verbs covering *discover → delegate → coordinate → share → observe*.
...
| `mx_create_task` / `mx_update_task` / `mx_list_tasks` | `task.create/update/list/graph` | Author and read the shared, durable plan (DAG) — **live (T301)**: create with `depends_on`/`blocks`, list returns the DAG (nodes + edges), update transitions state; a node's signed `action` is **authored, not dispatched** (dispatched via `mx_dispatch_task`, T303) |
| `mx_dispatch_task` *(guarded)* | `call.start` / `exec.start` (via `mx_delegate_tool` / `mx_run_command`) | **Dispatch a node's authored, signed `action`** through the full authorize pipeline — **live (T303)**: a `kind: 'tool'` action routes through `call.start`, a `kind: 'exec'` action through `exec.start`, so the dispatch is indistinguishable on the wire from a direct delegation/exec and re-runs sig → trust → policy → sandbox → approval on the receiver. Authoring an action is never authorizing it; a task-stable `idempotency_key` makes re-dispatch (a restart per T302/T304) dedupe rather than double-execute |
...
> **Hands-on companion:** for the single copy-pasteable setup per runtime (ADK / Claude / OpenCode / Pi / custom), each cross-referenced to the gated e2e arm that verifies it, see the [Runtime integration guide](./runtime-integration.md). This section is the narrative; the guide is the task-oriented recipe.
...
- **Resolved by T204 ([decision record](pi-tool-surface-capability.md)): Pi has no built-in MCP client today, so the Pi binding uses Pi's *native tool-registration* API** (SDK `customTools` / `defineTool`, extension-time `pi.registerTool()`) — it cannot consume `@mx-loom/mcp` directly the way ADK (`MCPToolset`) or OpenCode (`mcp` entry) can. MCP for Pi remains only a possible *future, extension-mediated* path (Pi blesses "MCP server integration" as something an extension can add, but mx-loom would have to build that Pi-side MCP client first). So T205 generates Pi `ToolDefinition[]` from the canonical descriptors (Pi `parameters` are TypeBox, so `enum` fields emit `StringEnum` for Google-provider compatibility) and routes execution through the same registry handlers + toolbelt seam. Because mx-agency already wraps Pi behind `AgentRunner`, the Pi binding stays a thin map from the canonical registry to Pi's tool type. *(**Landed by T205 ([`@mx-loom/pi`](../packages/pi/README.md)):** the binding mechanism — a fail-closed JSON Schema → TypeBox converter (`enum` → `StringEnum`, never `Type.Union`; throws at build, never widens to `Type.Any()`), a **locally reimplemented** dispatch/context (no `@modelcontextprotocol/sdk` in Pi's dep graph), `createPiToolDefinitions`/`registerMxTools`/`createMxPiExtension`, and a serializer that carries the T102 envelope in **both** `content` and `details`. The `withAudit` tap fires once at the `execute` chokepoint. `typebox`/`@earendil-works/pi-ai`/`@earendil-works/pi-coding-agent` are optional `peerDependencies` with the Pi/TypeBox types mirrored locally and the `Type`/`StringEnum` builders **injected by the host** (single TypeBox runtime; no heavy/native dep leak; type-checks without the peer). No new authority surface, no `bin`, secret-free. The 331-test daemon-free suite (Testing Plan #1–#9) is in `packages/pi/test/smoke.test.ts`; the live "a Pi agent calls `mx_delegate_tool`" arm is staged behind `MXL_PI_BINDING_E2E=1` + the two-daemon fixture, at `packages/golden/test/t205-pi-binding.e2e.test.ts` (skip-clean without, fail-not-skip in CI when requested). Deferred results stay model-driven via `mx_await_result` — no hidden poll loop ships.)*
...
- **Task state.** The DAG (`com.mxagent.task.v1`) is the durable, shared plan — `proposed→pending→assigned→executing→succeeded/failed`, with `depends_on`/`blocks` and signed `action`. The runtime owns how to think about the plan; MX owns the plan of record. This is also the crash-recovery boundary: a runtime can die and a new one resumes from task state. *(Implemented in T301: the three model-facing task verbs `mx_create_task` / `mx_update_task` / `mx_list_tasks` in `@mx-loom/registry` let cognition **write** and **read** this DAG — create a task with dependencies, list a DAG (nodes + edges) that reflects them, and transition a task's state through the daemon. The model-facing `TaskState` normalises the daemon vocabulary via a stable `mapTaskState` table (`unknown` is the documented safe fallback). A node's signed `action` is **authored** into the record by T301 and **dispatched** through the full authorize pipeline by **T303 (landed)**: `mx_dispatch_task` resolves a node's action (`task.list` + id filter), maps it via the shared `actionToDispatch` mapper — the single source of truth that proves what was authored is exactly what is dispatched — and re-routes it through `mx_delegate_tool` (`call.start`, a `kind: 'tool'` action) or `mx_run_command` (`exec.start`, a `kind: 'exec'` action), so the dispatch traverses the identical receiver-side sig → trust → policy → sandbox → approval pipeline (authoring an action is never authorizing it; a task-stable `idempotency_key` makes a re-dispatch on restart dedupe rather than double-execute); **T302 landed** (crash-recovery toolbelt layer: `SessionDescriptor` is the non-secret restart boundary; `resumeSession()` re-registers idempotently (same room + `correlation_id`) and reconstructs the durable plan into a `PlanSnapshot` via `reconstructPlan()`; `watchTasks()` subscribes with a clamped-interval poll fallback, cursor+signature dedup, and stream-gap recovery; the AC "a killed-and-restarted runtime resumes the plan from task state" is satisfied on the verified `task.list` surface; the push `task.watch` backend is a one-const swap gated until pinned — see the toolbelt README); the multi-agent-restart exit test is T304. The `task.*` wire shapes are pinned at the two-daemon round-trip; mx-loom forks no daemon-side task storage.)*
...
- **Canonical registry** with the **9 M1 verbs**: `mx_find_agents`, `mx_describe_agent`, `mx_delegate_tool`, `mx_run_command` (guarded, off by default), `mx_await_result`, `mx_share_context`, `mx_get_context`, `mx_cancel`, `mx_workspace_status`. (The **3 task DAG verbs** — `mx_create_task` / `mx_update_task` / `mx_list_tasks` — landed in **Phase 3 (T301)**, growing the canonical set to **12**, and the dispatch verb **`mx_dispatch_task`** landed in **T303**, bringing it to **13**; `mx_workspace_status` surfaces agents + project and leaves its reserved `tasks` dimension intentionally unpopulated — a dedicated `mx_list_tasks` is the cleaner surface.)
...
| **2 — Universal binding** | ADK / OpenCode / Pi | ADK `MCPToolset` + `LongRunningFunctionTool` shim; OpenCode `mcp` entry; Pi binding via **native tool registration** (T204 decision — Pi has no built-in MCP client; **landed T205, `@mx-loom/pi`**) — all from the same descriptor set; **portability matrix landed T206** (`packages/golden/test/portability-matrix.e2e.test.ts` — the realised M2-exit gate) | Same golden test passes under ADK, OpenCode, Pi — exercised by the T206 portability matrix: Pi + ADK drive the full S1–S8, OpenCode the always-on descriptor-identity invariant + S3 under a model |
| **3 — Coordination depth** | Plans + context as tools | Task DAG tools (`mx_create/update/list_tasks`) — **landed (T301)**, the canonical set grows 9 → 12; `task.watch` resumption — **landed (T302)**: `SessionDescriptor` / `PlanSnapshot` / `resumeSession()` / `watchTasks()` in the toolbelt; signed task-action dispatch — **landed (T303)**: `mx_dispatch_task` runs a node's authored action through the identical authorize pipeline (canonical set → 13); multi-agent plan with restart (T304) is the M3 exit (`mx_cancel` shipped in Phase 1 via T108) | A multi-agent plan executes across ≥2 agents with durable task state surviving a runtime restart |
