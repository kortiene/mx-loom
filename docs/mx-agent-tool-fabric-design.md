...
| **2 — Universal binding** | ADK / OpenCode / Pi | ADK `MCPToolset` + `LongRunningFunctionTool` shim; OpenCode `mcp` entry; Pi binding via **native tool registration** (T204 decision — Pi has no built-in MCP client; **landed T205, `@mx-loom/pi`**) — all from the same descriptor set | Same golden test passes under ADK, OpenCode, Pi |
