...
| **3 — Coordination depth** | Plans + context as tools | Task DAG tools (`mx_create/update/list_tasks`) — **landed (T301)**, the canonical set grows 9 → 12; `task.watch` resumption (T302) + signed task-action dispatch (T303) follow (`mx_cancel` shipped in Phase 1 via T108) | A multi-agent plan executes across ≥2 agents with durable task state surviving a runtime restart |
